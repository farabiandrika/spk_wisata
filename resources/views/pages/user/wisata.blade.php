                ...
                <table class="table">
                  ...
                  <tbody>
                    @foreach ($wisatas as $wisata)
                        <tr>
                          <th scope="row">{{ $wisata->nama }}</th>
                          @for ($i = 0; $i < count($kriterias); $i++)
                          <td>
                            <select required class="form-control" name="{{ $wisata->id }}_pilihan_{{ $kriterias[$i]->id }}">
                              <option value="" selected disabled>Pilih</option>
                              @foreach ($kriterias[$i]->subKriterias as $sub_kriteria)
...
<div class="modal fade" id="hasil-perhitungan" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    ...
    <script type="text/javascript">
        ...
        // Next Step
        $(document).on('submit','#wisata', function(e) {
            e.preventDefault()
